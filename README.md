...
Version **0.2.1** validates API results before rendering: all five sections, bounded plain text and valid source/page references are required. Empty sections show **未提供 / Not provided**. Malformed results show a retryable error, while an existing valid summary stays visible. Use integration **0.4.2** for MiniMax-M3 reasoning separation, schema negotiation, bounded format retry and incomplete-response detection. OpenAI-compatible models still need image-input support; consistent display is not a guarantee of factual accuracy.
